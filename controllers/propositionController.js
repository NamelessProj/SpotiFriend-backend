...
const addProposition = asyncHandler(async (req, res) => {
    ...
    await sendEmail(room.owner.email, `New Proposition - ${room.name}`, `<h2>Room - ${room.name}</h2><p>A new proposition was sent.</p><br/><p>Song name: <b>${song.name}</b></p><p>Song uri: <b>${song.uri}</b></p><p>Song url: <b>${song.external_urls.spotify}</b></p><p>Artists: <b>${artists}</b></p><p>Song href: <b>${song.href}</b><p>Song is explicit: <b>${song.explicit}</b></p></p>`);
});
